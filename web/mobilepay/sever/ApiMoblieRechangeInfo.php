...
/*
* 作者 Administrator
* 创建于 2014-05-05 上午7:44
* 描述：
*/


class ApiMoblieRechangeInfo extends TfbxmlResponse {
    //读取充值面额
    ...
    //读取快捷支付银行卡列表
    public function readRechabkcardinfo() {
        $db = new DB_test();
        $arr_header = $this->arr_header;
        $arr_body = $this->arr_body;
        $arr_channelinfo = $this->arr_channelinfo;
        ...
        $query = "select fd_bankcard_id as rchabkcardid ,fd_bankcard_no as rchabkcardno,
                  fd_bankcard_bankname as rechabkcardbank
                   from tb_bankcard where fd_bankcard_authorid = '$authorid' and fd_bankcard_active = '1' ";
        $db->query($query);
        $arr_msg = auto_charset($db->getData('', 'msgbody'), 'gbk', 'utf-8');
    ...
    function RechaMoneyRq() {
        ...
        $rechamobile      = trim(u2g($arr_body['rechamobile']));    //充值手机号码
        $rechamobileprov  = trim($arr_body['rechamobileprov']);    //手机所属地区
       ...
       // $paycardid = trim(GetPayCalcuInfo::readpaycardid($paycardid)); //刷卡器设备号
        ...
        $payfee  =  $this->readRechacostmoney($rechamoney);
        ...
        $datadetailArray['fd_mrclist_rechaphone'] = $rechamobile;
        $datadetailArray['fd_mrclist_paymoney'] = $rechapaymoney;
        $datadetailArray['fd_mrclist_mobileprov'] = u2g($rechamobileprov);
        ...
        $db->insert("tb_mobilerechargelist", $datadetailArray);
    ...
    public function checkRechaMoneyStatus() {
        ...
        $query = "select fd_mrclist_bkordernumber as bkordernumber,fd_mrclist_authorid as authorid ,
                  ...
                  fd_mrclist_rechaphone as rechaphone
    ...
    public function readMobileRechangelist() {
        ...
        $query = "select  1 from  tb_mobilerechargelist where  fd_mrclist_authorid = '$authorid' and" .
            "  (fd_mrclist_payrq = '00' )";
        $db->query($query);
        $msgallcount = $db->nf();
        ...
        else '".$arr_state[3]."' END  state,case
        ...
        else '".$arr_ofstate[3]."' END  ofstate from  tb_mobilerechargelist
         where fd_mrclist_authorid = '$authorid'
