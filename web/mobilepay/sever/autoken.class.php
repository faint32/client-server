...
class AuToken {
	...
	function checkauthorModqx($api_name, $api_name_func) //检查该功能需要的用户权限 
	{
		$db = new DB_test();
	...
	function checkReqToken($reqtoken, $autoken, $apiname, $apinamefunc) //验证授权码
	{
		...
		if ($db->execute($query)) {
			...
			$Error = array (
				'result' => 'failure',
				'retcode' => '300',
				'retmsg' => $reqtoken.'您的登录信息已被变更，为保证安全请重新登录'
			);
			$this->ErrorReponse->reponError($Error); //出错反馈 
			exit;
		}
