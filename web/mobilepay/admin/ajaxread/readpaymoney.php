...
$aColumns = array("fd_saler_truename","fd_paycard_datetime","fd_paycard_no","fd_bank_name","fd_author_truename");
...
if($db->nf())
{
	...
	{
	   $vid             = $db->f(fd_paycard_id);            //id∫≈  
	   $vno             = $db->f(fd_paycard_no);            //id∫≈  
	  ...
	  if($vstatus==1)
	  {
		...
		{
			$vpaycardtype="–≈”√ø®";
		}elseif($vpaycardtype=="bankcard"){$vpaycardtype="¥¢–Óø®";}
	   ...
	   $vedit ='<a href="paycardmsg.php?listid='.$vid.'" class="edit" >…Û∫À</a>';
	   	$count++;   
