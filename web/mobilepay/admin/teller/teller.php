...
switch ($action){
    case "delete":   // 记录删除
       $query = "delete from web_teller where fd_tel_id = '$id'";
       $db->query($query);
       Header("Location: $gotourl");       
	break;
    case "new":   // 进行编辑，要提供数据
       $query = "select * from web_teller where fd_tel_name ='$name' ";
	     $db->query($query);
       if($db->nf()>0){
	         $error = "此用户名已经存在，请重新输入！";
       }else{
           $pass = md5($GLOBALS["pass"]) ;
           $query="INSERT INTO web_teller(
    ...
    case "edit":   // 修改记录
        if(!empty($pass) and !empty($mailpass)){
           ...
           $query = " update web_teller set
 	    	              fd_tel_name = '$name'  , fd_tel_pass ='$pass'  , fd_tel_error ='$errornum'   ,
          ...
          $query = " update web_teller set
    	    ...
    	    $mailpass = bin2hex($mailpass) ;  
    	    $query = " update web_teller set
 	    	             fd_tel_name = '$name'  , fd_tel_error ='$errornum'   ,
