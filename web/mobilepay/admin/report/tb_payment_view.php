...
//获取信用卡还款数据
$query = "select fd_ccglist_id,fd_ccglist_authorid,fd_ccglist_paycardid,sum(fd_ccglist_paymoney) as paymoney,DATE_FORMAT(fd_ccglist_paydate,'%Y-%m-%d') as  paydate   from tb_creditcardglist where (fd_ccglist_paydate>='$startday' and fd_ccglist_paydate<='$endday') and fd_ccglist_payrq = '00' and fd_ccglist_state = '9' $querydate $isnodata group by paydate";
$db->query($query);
while($db->next_record()){
...
//获取优惠券数据
$query = "select fd_couponsale_id,fd_couponsale_authorid,fd_couponsale_paycardid,DATE_FORMAT(fd_couponsale_datetime,'%Y-%m-%d') as  paydate  ,sum(fd_couponsale_money) as paymoney from tb_couponsale where (fd_couponsale_datetime>='$startday' and fd_couponsale_datetime<='$endday') $querydate $isnodata  group by paydate";
$db->query($query);
while($db->next_record()){
  $arrCouponglideData[$db->f('paydate')] = array('id'=>$db->f('fd_couponsale_id'),'authorid'=>$db->f('fd_couponsale_authorid'),
                                'paycardid'=>$db->f('fd_couponsale_paycardid'),'paymoney'=>$db->f('paymoney'),
								'paydate'=>$db->f('paydate')
								); 
}
