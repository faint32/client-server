...
switch($action){
	case "del":   //删除细节表数据
	  for($i=0;$i<count($checkid);$i++){
        if(!empty($checkid[$i])){
			...
			$query = "select * from tb_salelistbackdetail  where fd_stdetail_id='$checkid[$i]'";
			$db->query($query);
			if ($db->nf()) {
			...
			$query="delete from tb_salelistbackdetail where fd_stdetail_id = '$checkid[$i]'";
			$db->query($query);
			
			
			$query="delete from tb_salelist_tmp where fd_tmpsale_seltid = '$checkid[$i]' and fd_tmpsale_type='saleback'";
      	    $db->query($query);
        }
	...
	echo "<script>alert('删除成功!');location.href='jxcsaleback.php?listid=$listid';</script>";
		case "del_one":   //删除细节表数据

			$old_paycardid=getdatepaycard("tb_salelistbackdetail","stdetail",$delseltid);
			...
			$query="update  tb_salelistbackdetail set fd_stdetail_paycardid='$del_paycardid', fd_stdetail_quantity='$paycardnum' 
			where fd_stdetail_id = '$delseltid'";
			$db->query($query);
			
			
			$query="update  tb_salelist_tmp set fd_tmpsale_paycardid='$del_paycardid'  where fd_tmpsale_seltid = '$delseltid' and fd_tmpsale_type='saleback'";
	...
	echo "<script>alert('删除成功!');location.href='jxcsaleback.php?listid=$listid';</script>";
	  ...
	  case "new":  //新增数据
		 if (empty($listid)){  //如果单据id是不存在的
	  		...
	  		listnumber_update(4);  //保存单据
	  	
	  	$query = "select * from tb_salelistback where fd_selt_no = '$listno' ";
	  	$db->query($query);
	  	if($db->nf()){
	  		$error = "单据编号已经存在！请查证！";
	    }else{
	      
	      $query = "insert into tb_salelistback(
	                fd_selt_no          ,   fd_selt_date  ,fd_selt_cusid   , fd_selt_cusno,
					fd_selt_cusname     ,	fd_selt_skfs  ,fd_selt_shaddress,fd_selt_ldr
					,fd_selt_dealwithman  
	                )values(
	                '$listno'           ,   '$date'      ,'$cusid'         , '$cusno',
					'$cusname'          ,   '$skfs'      ,'$shaddress','$ldr'
					,'$dealwithman'		
	                )";
	    ...
	    $query = "select * from tb_salelistback where fd_selt_no = '$listno' and fd_selt_id <> '$listid' ";
	  	$db->query($query);
	  	if($db->nf()){
	  		$error = "单据编号已经存在！请查证！";
	    }else{
	   ...
	   echo "<script>alert('暂存成功!');location.href='jxcsaleback.php?listid=$listid';</script>";
	  break;
}
...
switch($end_action){
	case "endsave":    //最后提交数据
	      $query = "update tb_salelistback set 
	                ...
	                where fd_selt_id = '$listid' ";		
	      $db->query($query);   //修改单据资料
	    		$query = "select * from tb_salelistbackdetail where fd_stdetail_seltid = '$listid' ";
				$db->query($query);
			while($db->next_record())
			{
				$stdetail_id=$db->f(fd_stdetail_id);
					$query="delete from tb_salelist_tmp where fd_tmpsale_seltid = '$stdetail_id' and fd_tmpsale_type='saleback'";
					$db->query($query);
			}
		$query = "update tb_salelistback set 
		  fd_selt_state  = 1   
	  ...
	  break;
		...
		$query = "delete from tb_salelistback where fd_selt_id = '$listid'";
	     $db->query($query);   //删除总表的数据
	     
	     $query = "delete from tb_salelistbackdetail where fd_stdetail_seltid = '$listid' ";
...
$t->set_file("salelist","jxcsaleback.html"); 
//商品名称
...
{		// 新增
   ...
   $db->query($query);
      if($db->nf()){
	   ...
	   $dealwithma           = $db->f(fd_selt_dealwithman);          //经手人
