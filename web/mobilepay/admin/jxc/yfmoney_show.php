...
$t = new Template('.', "keep");
$t->set_file("yfmoney_show","yfmoney_show.html");
$t->set_block("yfmoney_show", "BXBK", "bxbks");


//查询所有供应商应付款
$query = "select fd_ysyfm_companyid , fd_ysyfm_type , fd_ysyfm_money from tb_ysyfmoney 
          left join tb_supplier on fd_supp_id = fd_ysyfm_companyid
          where fd_ysyfm_money<0 and fd_ysyfm_type = '2' ";
$db->query($query);
if($db->nf()){
...
//查询所有客户应付款
$query = "select fd_ysyfm_companyid , fd_ysyfm_type , fd_ysyfm_money from tb_ysyfmoney 
          left join tb_customer on fd_cus_id = fd_ysyfm_companyid
          where fd_ysyfm_money<0 and fd_ysyfm_type = '1' ";
$db->query($query);
if($db->nf()){
	while($db->next_record()){
		$linkid     = $db->f(fd_ysyfm_companyid);
...
while(list($key,$val)=@each($arr_client)){
     ...
     $strurl = "currentaccount_show.php?clienttype=".$custype."&cusid=".$linkid."&backurl=yf";

	   $ysmoney = number_format($ysmoney, 2, ".", "");
	   if($ysmoney>0){  
		   $count++ ;
		   $t->set_var(array("linkname"	   => $linkname   ,
		   		              "linktype"     => $linktype   ,
		   		              "linkno"       => $linkno     ,
		   		              "linkid"       => $linkid     ,
		   		              "custype"      => $custype    ,
...
$t->pparse("out", "yfmoney_show");    # 最后输出页面
