...
if($db->nf()){
  ...
  $t->set_file("sale_sq_view","saleback_sq_view.html"); 
//商品名称
...
if($db->nf()){
	...
	while($db->next_record()){		
      ...
      // $vallmoney +=$vmoney;
	   if($strpaycardid){$strpaycardid=",".$vpaycardid;}else{$strpaycardid=$vpaycardid;}
...
if($db->nf())
{
	while($db->next_record())
	{
		...
		$arr_savesaleprice[$db->f(fd_paycard_key)]=$db->f(fd_paycard_saleprice);
	}
}

$arr_savepaycard=explode(",",$strpaycardid);
print_r($arr_savesaleprice);
foreach($arr_savepaycard as $value)
{
	$vallmoney +=$arr_savesaleprice[$value];
} */

$vallmoney=getdallsaleprice($strpaycardid);//刷卡器销售总额
