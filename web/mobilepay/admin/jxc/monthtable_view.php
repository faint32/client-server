...
$db = new DB_test ;
$dbtj = new DB_test ;
$gourl = "monthtable_query.php";
$gotourl = $gourl . $tempurl;
require("../include/alledit.1.php");
$t = new Template('.', "keep");
$t->set_file("monthtable_view","monthtable_view.html");
$t->set_block("monthtable_view", "BXBK", "bxbks");

$begindate = date( "Y/m/d" , mktime(0,0,0,$month,1,$year) );
$enddate = date( "Y/m/d" , mktime(0,0,0,$month+1,0,$year) );

//统计工资单
/*$query = "select * from  tb_wagelistdetail left join tb_wagelist on fd_wagelist_id = fd_wagelistdetail_wlid left join tb_laborageitem on fd_wagelistdetail_leimid = fd_leim_id where fd_wagelist_date >= '$begindate' and fd_wagelist_date <= '$enddate'" ;
$db->query($query);
if($db->nf()){
  while($db->next_record()){
       $listdate  = $db->f(fd_wagelist_date); 
...
//查看月往来数
/*$query = "select COUNT(DISTINCT fd_selt_cusid) as cusnum , fd_selt_date from tb_salelist
          where fd_selt_date >= '$begindate' and fd_selt_date <= '$enddate' and fd_selt_state =1 
          and fd_selt_iskickback = 0  group by fd_selt_date " ;
$db->query($query);
if($db->nf()){
	while($db->next_record()){
		$wlcusnum  = $db->f(cusnum);
		$listdate   = $db->f(fd_selt_date);        //单据时间
...
$query = "select sum(fd_stock_allmoney) as allmoney  , fd_stock_date 
          from tb_stock  
          where fd_stock_date >= '$begindate' and fd_stock_date <= '$enddate' and fd_stock_state =2 
...
//进货单
$query = "select sum(fd_stock_allmoney) as allmoney  , fd_stock_date from tb_paycardstock where fd_stock_date >= '$begindate' and fd_stock_date <= '$enddate' and fd_stock_state =9 group by fd_stock_date";
$db->query($query);
if($db->nf()){
	while($db->next_record()){
		$cgmoney    = $db->f(allmoney);
...
//退货单
$query = "select sum(fd_stock_allmoney) as allmoney  , fd_stock_date from tb_paycardstockback where fd_stock_date >= '$begindate' and fd_stock_date <= '$enddate' and fd_stock_state =9 group by fd_stock_date";
$db->query($query);
if($db->nf()){
	while($db->next_record()){
		$cgmoney    = $db->f(allmoney);
...
$t->pparse("out", "monthtable_view");    # 最后输出页面
?>