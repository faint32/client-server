...
$query = "SELECT * FROM tb_salelistback WHERE fd_selt_id = '$listid'";
         ...
         $id         = $db->f(fd_sebk_id);            //id号  
         $no         = $db->f(fd_sebk_no);            //单据编号
         $cusid      = $db->f(fd_sebk_cusid);         //客户id
         $cusno      = $db->f(fd_sebk_cusno);         //客户编号 
         $cusname    = $db->f(fd_sebk_cusname);       //客户名称
         $now        = $db->f(fd_sebk_date);          //录单日期
         $dept       = $db->f(fd_dept_name);          //部门
         $staid      = $db->f(fd_sebk_staid);         //录单人ID
         $storage    = $db->f(fd_storage_name);       //仓库
         $memo       = $db->f(fd_sebk_memo);          //备注
         $state      = $db->f(fd_sebk_state);         //状态
         $dealwithman= $db->f(fd_sebk_dealwithman);   //经手人ID
         $temp_arr   = explode("-",$now);
         $year       = $temp_arr[0];
         $month      = $temp_arr[1];
         $day        = $temp_arr[2];

$t->set_file("salebackview","salebackprint.html"); 
...
$query = "SELECT * FROM tb_salelistbackdetail WHERE fd_stdetail_id = '$listid'";
$db->query($query);
...
$t->pparse("out", "salebackview");    # 最后输出页面
?>