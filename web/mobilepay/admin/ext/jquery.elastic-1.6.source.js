	...
	$.fn.extend({  
		elastic: function() {
			...
			return this.each( function() {
					...
					if(textareaContent+'&nbsp;' != twinContent){
						...
						if(Math.abs($twin.height()+lineHeight - $textarea.height()) > 3){
							...
							if(goalheight >= maxheight) {
								setHeightAndOverflow(maxheight,'scroll');
							} else if(goalheight <= minheight) {
								setHeightAndOverflow(minheight,'scroll');
							} else {
								setHeightAndOverflow(goalheight,'scroll');
							}
				...
				$textarea.css({'overflow':'scroll'}).bind('focus',function() {
