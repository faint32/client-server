...
//获取刷卡器数据
$query = "select * from tb_author left join tb_paycard on (fd_paycard_authorid = fd_author_id  and fd_paycard_active = '1') where fd_author_state = '9' and fd_author_isstop = '0' $querywhere  $isnodata";

$db->query($query);
while($db->next_record()){
  $arrPaycardData[] = array('id'=>$db->f('fd_paycard_id'),'key'=>$db->f('fd_paycard_key')); 
...
$query = "select fd_repmglist_id,fd_repmglist_authorid,fd_repmglist_paycardid,sum(fd_repmglist_paymoney) as paymoney,DATE_FORMAT(fd_repmglist_paydate,'%Y-%m-%d') as  paydate from tb_repaymoneyglist where fd_repmglist_authorid = '".$searchvalid."' and (fd_repmglist_paydate>='$startday' and fd_repmglist_paydate<='$endday') and fd_repmglist_payrq = '1' and fd_repmglist_state = '9' $querydate $isnodata  group by fd_repmglist_paycardid,paydate";
$db->query($query);
while($db->next_record()){
  
  $arrRepaymoneygData[$db->f('paydate').'-'.$db->f('fd_repmglist_paycardid')] = array('id'=>$db->f('fd_repmglist_id'),'authorid'=>$db->f('fd_repmglist_authorid'),
...
$query = "select fd_tfmglist_id,fd_tfmglist_authorid,fd_tfmglist_paycardid,sum(fd_tfmglist_paymoney) as paymoney,DATE_FORMAT(fd_tfmglist_paydate,'%Y-%m-%d') as  paydate  from tb_transfermoneyglist where fd_tfmglist_authorid = '".$searchvalid."' and (fd_tfmglist_paydate>='$startday' and fd_tfmglist_paydate<='$endday') and fd_tfmglist_payrq = '1' and fd_tfmglist_state = '9' $querydate $isnodata group by fd_tfmglist_paycardid,paydate";
$db->query($query);
while($db->next_record()){
  $arrTransfermoneygData[$db->f('paydate').'-'.$db->f('fd_tfmglist_paycardid')] = array('id'=>$db->f('fd_tfmglist_id'),'authorid'=>$db->f('fd_tfmglist_authorid'),
...
//获取信用卡还款数据
$query = "select fd_ccglist_id,fd_ccglist_authorid,fd_ccglist_paycardid,sum(fd_ccglist_paymoney) as paymoney,DATE_FORMAT(fd_ccglist_paydate,'%Y-%m-%d') as  paydate   from tb_creditcardglist where fd_ccglist_authorid = '".$searchvalid."' and (fd_ccglist_paydate>='$startday' and fd_ccglist_paydate<='$endday') and fd_ccglist_payrq = '1' and fd_ccglist_state = '9' $querydate $isnodata group by fd_ccglist_paycardid,paydate";
$db->query($query);
while($db->next_record()){
  $arrCreditcardgData[$db->f('paydate').'-'.$db->f('fd_ccglist_paycardid')] = array('id'=>$db->f('fd_ccglist_id'),'authorid'=>$db->f('fd_ccglist_authorid'),
...
$query = "select fd_couponglide_id,fd_couponglide_authorid,fd_couponglide_paycardid,DATE_FORMAT(fd_couponglide_datetime,'%Y-%m-%d') as  paydate  ,sum((fd_couponglide_addmoney) - (fd_couponglide_lessmoney)) as paymoney from tb_couponglide where fd_couponglide_authorid = '".$searchvalid."' and (fd_couponglide_datetime>='$startday' and fd_couponglide_datetime<='$endday') $querydate $isnodata group by fd_couponglide_paycardid,paydate";
$db->query($query);
while($db->next_record()){
  $arrCouponglideData[$db->f('paydate').'-'.$db->f('fd_couponglide_paycardid')] = array('id'=>$db->f('fd_couponglide_id'),'authorid'=>$db->f('fd_couponglide_authorid'),
...
if($isnodata2 == 'N'){
	while(true){
	  ...
	  while(list($key,$val) = each($arrPaycardData)){
		...
		$arrTemData = array('paycardid'=>$val['key'],'Repaymoneygpaymoney'=>$Repaymoneygpaymoney,'Transfermoneygpaymoney'=>$Transfermoneygpaymoney,
