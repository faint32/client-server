...
if(!empty($action) or !empty ($end_action) or !empty ($listid)) {
	$query = "select * from tb_paymoneylist where fd_pymylt_id = '$listid' and fd_pymylt_state != 3 ";
	$db->query($query);
	if($db->nf()){
		echo "<script>alert('该单据已经不在本步骤，不能再修改，请查证');location='$gotourl'</script>";		
		$action = "";
		$end_action = "";
	}
}
...
switch ($end_action) {
	case "endsave" :
			...
			$query = "update tb_paymoneylist set fd_pymylt_state =9, fd_pymylt_datetime = now()
...
$t->set_file ( "paymoneylist_hd", "paymoneylist_hd.html" );
...
if($db->nf()){
 ...
 $query = "select fd_couponset_fee as fee from tb_couponset left join tb_arrive on fd_arrive_id =fd_couponset_arriveid";
		$arr_couponset = $db->get_one($query);
		$couponfee = substr($arr_couponset['fee'], 0, -1); //浮动手续费 
$t->set_block ( "paymoneylist_hd", "prolist", "prolists" );
$query = "select 
                 case
...
$t->pparse ( "out", "paymoneylist_hd" ); # 最后输出页面
?>