...
class tb_paymoneylist_b extends browse {
	var $prgnoware = array ("资金代付管理", "代付款申请" );
	...
	var $browse_delsql = "delete from tb_paymoneylist where fd_pymylt_id = '%s'";
	
	var $browse_new    = "paymoneylist.php";
	var $browse_edit   = "paymoneylist.php?listid=";

	var $browse_relatingdelsql = array ("1" => "delete from tb_payinmoneyfpdetail where fd_pymyltdetail_paymoneylistid = '%s'" );
	
	var $browse_state = array("fd_pymylt_state"); 
	
	var $browse_defaultorder = " CASE WHEN fd_pymylt_state = '0'
                                THEN 1                                  
   	  ...
   	  switch($this->arr_spilthfield[0]){  //判断记录是在那一个状态下，在那一个状态下就使用那一个连接
   	  	case "0":
  	        ...
  	        $returnval = "<a href=javascript:linkurl(\"".$this->browse_edit.$key."\")><font color='#0000ff'>".$this->browse_editname."</font></a>" ;
   	  	  ...
   	  	  break;
   	  	default:
   	  	  $returnval = "<a href=javascript:linkurl(\"paymoneylist_view.php?backstate=0&listid=".$key."\")>查看</a>" ;;	
   	  	  break;
   	  }
  	  return $returnval;
   }
