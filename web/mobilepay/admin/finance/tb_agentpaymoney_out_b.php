...
class tb_paymoneylist_b extends browse {
	var $prgnoware = array ("资金代付管理", "代付款出账" );
	...
	// var $browse_edit   = "paymoneylist_cz.php?listid=";
  var $browse_edit   = "../agentbank/createfile.php?listid=";
	
	var $browse_state = array("fd_pymylt_state"); 
	
	var $browse_defaultorder = " CASE WHEN fd_pymylt_state = '2'
                                THEN 1                                  
                                WHEN fd_pymylt_state = '0'
                                THEN 3 
                                WHEN fd_pymylt_state = '1'
                                THEN 2
                                WHEN fd_pymylt_state = '3'
                                THEN 4                                 
                                END,fd_pymylt_state desc
                              ";	  
	
	var $browse_editname = "打印出账资金账单";
   	  ...
   	  switch($this->arr_spilthfield[0]){  //判断记录是在那一个状态下，在那一个状态下就使用那一个连接
   	  	case "2":
  	        ...
  	        $returnval = "<a href=javascript:linkurl(\"".$this->browse_edit.$key."\")><font color='#0000ff'>".$this->browse_editname."</font></a>" ;
   	  	  ...
   	  	  break;
   	  	default:
   	  	  $returnval = "<a href=javascript:linkurl(\"paymoneylist_view.php?backstate=2&listid=".$key."\")>查看</a> &nbsp;|&nbsp;&nbsp;&nbsp;" ;
		  //$returnval .= "<a href=javascript:linkurl(\"listprint.php?listid=".$key."\")>打印</a>" ;	
		  $returnval .= "<a href=javascript:windowopen(\"listprint.php?listid=".$key."\")>打印</a>" ;
   	  	  break;
   	  }
  	  return $returnval;
   }
...
//    			    );
//    var $bwlk_prgname = "../agentbank/createfile.php?listid=";
//    var $bwlk_title ="打印资金账单";  
// }
...
// $tb_paymoneylist_b_bu->browse_link  = array("lk_view0");
