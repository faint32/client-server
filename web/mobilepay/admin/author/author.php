...
switch($action){
	case "couponedit":
	  $query="update tb_author set 
			  fd_author_couponstate='$couponstate'    where fd_author_id='$listid'";
	  $db->query($query);
	  if($couponstate==1)
	  {
	  $error = "抵用券银行卡号已经审批成功！"; 
	...
	break;
	 case "edit":
	   ...
	   $db->query($query);
	   $shouinfo['shoucardno'] 	=$shoucardno;
	   $shouinfo['shoucardbank'] 	=$shoucardbank;
	   $shouinfo['shoucardman'] 	=$shoucardman;
	   $shouinfo['shoucardmobile']=$shoucardphone;
	   
	   check_agentshouinfo($shouinfo,$listid);
	   if($userpw){
	    	$userpw = md5($userpw);
...
/*		$payfset = $db->f(fd_payfset_name);
		...
		$couponstate  = $db->f(fd_author_couponstate);
		if($isstop){
...
$t->set_var("couponstate", $couponstate);
...
function check_agentshouinfo($shouinfo,$authorid){
	$db = new DB_test;
	$query = "update tb_agentpaymoneylist set fd_agpm_shoucardno = '".$shouinfo['shoucardno']."',fd_agpm_shoucardbank = '".$shouinfo['shoucardbank']."',fd_agpm_shoucardman = '".$shouinfo['shoucardman']."',fd_agpm_shoucardmobile = '".$shouinfo['shoucardmobile']."' where fd_agpm_authorid = '$authorid' and (fd_agpm_paytype= 'coupon' or fd_agpm_paytype='recharge') and fd_agpm_isagentpay!= '1'";
	$db->query($query);
	
	$query = "update tb_couponsale  set fd_couponsale_shoucardno = '".$shouinfo['shoucardno']."',fd_couponsale_shoucardbank = '".$shouinfo['shoucardbank']."',fd_couponsale_shoucardman = '".$shouinfo['shoucardman']."',fd_couponsale_shoucardmobile = '".$shouinfo['shoucardmobile']."' where fd_couponsale_authorid = '$authorid'  and fd_couponsale_isagentpay!= '1'";
	$db->query($query);
