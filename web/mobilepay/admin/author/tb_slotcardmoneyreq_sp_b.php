...
class tb_slotcardmoneyreq_sp_b extends browse {
	var $prgnoware = array ("基础设置", "审批额度管理" );
	var $prgnowareurl = array ("", "" );
	
	var $browse_key = "fd_pmreq_id";
	var $browse_edit  = "slotcardmoneyreq_sp.php?listid=" ;
   var $browse_editname = "审批";
	var $browse_queryselect = "select *,(fd_pmreq_authorid) as restmoney,(fd_pmreq_authorid) as usemoney from tb_slotcardmoneyreq 
	...
	var $browse_field = array ("fd_pmreq_reqno","fd_author_truename","fd_scdmset_name",
								"fd_scdmset_scope","fd_scdmset_nallmoney","usemoney","fd_scdmset_sallmoney",
								"fd_pmreq_reqmoney" ,"restmoney", "fd_pmreq_reqdatetime","fd_pmreq_state");
...
class fd_scdmset_nallmoney extends browsefield {
 ...
 class usemoney extends browsefield {
	var $bwfd_fdname = "usemoney"; // 数据库中字段名称
	var $bwfd_title = "本月已用额度"; // 字段标题
	 function makeshow() {	// 将值转为显示值
		$this->bwfd_show=getusemoney($this->bwfd_value);
	    return $this->bwfd_show;
	}
}
...
class fd_pmreq_reqmoney extends browsefield {
 ...
 class restmoney extends browsefield {
	var $bwfd_fdname = "restmoney"; // 数据库中字段名称
	var $bwfd_title = "剩余申请额度"; // 字段标题
	function makeshow() {	// 将值转为显示值
		
		 $this->bwfd_show=getrestmoney($this->bwfd_value);
		  return $this->bwfd_show;
	}
} 
...
class lk_view0 extends browselink {
   ...
   var $bwlk_prgname ="slotcardmoneyreq_sp.php?listid=";
   var $bwlk_title ="审批";  
}  */
