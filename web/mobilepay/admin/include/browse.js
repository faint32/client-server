...
　return ajax;
   ...
   function editdata(url2){
      ...
      if (confirm("是否操作所有选定的资料？")) {
	    	checknum = 0;            
        selectvalue = "";
	      checkdd =   (form1.recodeto.value - form1.recodeform.value) + 1;   
   ...
   function submit_collect(){
    ...
    function newdata(url){
       ...
       if(tmpUrlPage[1]!=undefined){
       	 window.location = url+'&tempurl=' + tempurl;
       }else{
	       window.location = url+'?tempurl=' + tempurl;
	     }
    }
...
　            }
    ...
    function selectit(){
		...
		if(form1.urlpage.value == ""){
		document.getElementById("openiframe").style.display = "none";
...
function autoResizeIframe(){  //自动调整下拉列表的大少
	document.all.listdetailpage.style.height=window.listdetailpage.document.body.scrollHeight+30;
}

function showiframe(){
	  document.getElementById("listdetailpage").style.display = "";
     document.getElementById("openiframe").style.display = "none";
	 document.getElementById("closeiframe").style.display = "";
   
		
}
function disshowiframe(){
	 document.getElementById("listdetailpage").style.display = "none";
	 	  document.getElementById("closeiframe").style.display = "none";
	   document.getElementById("openiframe").style.display = "";

}
					

function selectxl(val){		
          for(var i=0;i<document.getElementById("whatdofindq").options.length;i++){			
            if(document.getElementById("whatdofindq").options[i].value==val){						
               document.getElementById("whatdofindq").options[i].selected='selected';
              }
           }
 }