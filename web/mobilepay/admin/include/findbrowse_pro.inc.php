...
class findbrowse {
   ...
   var $brow_tmpname = "findbrowse.ihtml" ;  //模板文件名   
   ...
   var $brow_isshowback = "" ;
   var $brow_backgotourl ="";
  
   var $brow_hidden = array() ; // 隐藏的值-----2005-10-31----------------
  ...
  function main($now,$act,$pagerows,$order,$upordown,$check,
    ...
    $this->t->set_var("isshowbacknone",$isshowbacknone);
    $this->t->set_var("backgotourl",$this->brow_backgotourl);
    $this->t->set_var("showhidden",$showhidden);   //-----2008-6-19-------
  ...
  function makehidden(){   //生成隐藏值
    	$hiddenvalue = "";
    	for($j=0;$j<count($this->brow_hidden);$j++){
    		$hiddenname = $this->brow_hidden[$j][0];
    		$keyfield = $this->brow_hidden[$j][1];
