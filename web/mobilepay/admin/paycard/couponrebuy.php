...
switch ($action){  
    ...
    if($db->nf()){                                            //判断查询到的记录是否为空
	      $db->next_record();                                   //读取记录数据 
			...
			$payfee	 = $db->f(fd_couponrebuy_sxfmoney);
			
		    $memo = "回购优惠劵".$couponno."支付金额".$money."元";
			
			changecoupon($listid,$no,"rebuy",$money,$couponno,$authorid,"",$memo);
			
			changeaccountno('in',$payfee,$paycardid,$memo,$paymode,'tb_couponrebuy',$listid);
...
$t = new Template(".","keep");
$t->set_file("couponrebuy","couponrebuy.html");
