...
{
	 var $prgnoware    = array("信用卡还款管理","信用卡还款支付复核");
	 ...
	 var $browse_edit = "creditcard_sp.php?listid=" ;
	 var $browse_editname = "审核";
	 
	 var $browse_defaultorder = "fd_ccglist_paydate desc
                             ";	  	  
  
   var $browse_state = array("fd_ccglist_payrq");
	   ...
	   function makeedit($key) {	// 生成编辑连接
			  ...
			  switch($this->arr_spilthfield[0]){  //判断记录是在那一个状态下，在那一个状态下就使用那一个连接
				case "00":
				  ...
				  break;
				default:
				  $returnval = "<a href=javascript:linkurl(\"creditcard_sp.php?type=error&gotype=sp&listid=".$key."\")>查看</a>" ;;	
				  break;
			  }
			  return $returnval;
			}
        
	 var $browse_field = array("tdcount","fd_ccglist_payrq","fd_author_truename","fd_ccglist_shoucardno","fd_ccglist_fucardno","fd_ccglist_paymoney","fd_ccglist_payfee","fd_ccglist_money","fd_ccglist_sdcrpayfeemoney","fd_ccglist_payfeedirct","fd_ccglist_sdcragentfeemoney","fd_ccglist_arrivedate","fd_sdcr_name","fd_ccglist_paydate","fd_paycard_no","fd_ccglist_memo","fd_ccglist_bkordernumber","fd_ccglist_bkntno","fd_ccglist_isagentpay","fd_ccglist_agentdate");
...
class fd_ccglist_payrq extends browsefield {
        var $bwfd_fdname = "fd_ccglist_payrq";	// 数据库中字段名称
...
$tb_feedback_b_bu->browse_querywhere = "fd_ccglist_state = '0'";

if(!empty($seldofile)){
  $tb_feedback_b_bu->browse_querywhere .= " and fd_ccglist_payrq = '$seldofile' ";
  $tb_feedback_b_bu->browse_haveselectvalue = $seldofile ;   //选中的值
}else{
  $tb_feedback_b_bu->browse_querywhere .= " and fd_ccglist_payrq = '00' ";
