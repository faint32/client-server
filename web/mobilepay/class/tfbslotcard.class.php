...
class AgentPayglist {
	public static function insertPayglist($xml, $bkntno, $listid, $listno, $paytype, $method, $arr_feeinfo) {
		...
		if ($paytype == 'coupon') {
			...
			if ($paytype == 'recharge') {
				...
				"fd_agpm_sdcrpayfeemoney,fd_agpm_cusid)values
				...
				"'$arrivedate'	,'$bkmoney'  ,'$sdcrid' ,'$sdcrpayfeemoney' ,'$cusid'
																															  )";
				$db->query($query);
			}
		return true;
	...
	static function getcouponSale($xml, $listno, $listid, $paytype = 'coupon', $bkntno, $arr_feeinfo) {
		...
		"'$payfeedirct'   ,'$arrivedate'  , '$bkordernumber'  , '$bkmoney'    ,'$sdcrid' ,'$sdcrpayfeemoney'  , '$cusid'
																				  )";
...
class GetPayCalcuInfo {
	...
	public static function readpaycardid($paycardkey,$authorid) {
		global $authorid;
		global $arr_limitauthorid;
		$ErrorReponse = new ErrorReponse();
		$db = new DB_test();
		$paycardkey = strtolower($paycardkey);
		$paycardkey = substr(str_replace("fff","",$paycardkey),0,11);

		// 未开通刷卡器，判断用户是否绑定代理商
        if($paycardkey=="")
        {
            $query = "select fd_author_bdagentid as cusid from tb_author where fd_author_id ='$authorid' ";
            if ($db->execute($query)) {
                $array_return = $db->get_one($query);
               // $array_return['fd_author_bdagentid'] = $paycardkey;
            }

        }
		$query = "select fd_paycard_id as paycardid,fd_paycard_key as paycardkey,fd_paycard_cusid as cusid
		          from tb_paycard where fd_paycard_key like '%$paycardkey%'";
		//echo $query;
		if ($db->execute($query)) {
			$array_return = $db->get_one($query);
		}
		if ($array_return['paycardid'] == '') {
			...
			{
                $array_return['paycardkey'] = $paycardkey;
			    return $array_return; // 需要开通-cai
			}
			$Error = array (
				'result' => 'failure',
				'retcode' => '200',
        ...
        return $array_return;
        exit;
		if (self :: CheckpayCard($array_return['paycardid'])) {  //检查刷卡器状态
			return $array_return;
		} else {
			$Error = array (
				'result' => 'failure',
				'retcode' => '200',
				'retmsg' => '未登记刷卡器，请联系客服激活该刷卡器['.$paycardkey.']'
			);
			$ErrorReponse->reponError($Error); //出错反馈 
		}
	}
	...
	public static function CheckpayCard($paycardid) {
		...
		$db = new DB_test();
		global $authorid;
		$query = "select fd_paycard_id,fd_paycard_active,fd_paycard_posstate from tb_paycard where   fd_paycard_id = '$paycardid'  ";
        //and  fd_paycard_active = '1' and fd_paycard_posstate <> '3' and fd_paycard_posstate <>'0' 刷卡器激活状态
		$db->query($query);
		if ($db->execute($query)) {
			return true;
		} else {
			...
			$Error = array (
				'result' => 'failure',
				'retcode' => '200',
				'retmsg' => '刷卡器异常，请联系客服激活该刷卡器['.$paycardkey.']'
			);
			$ErrorReponse->reponError($Error); //出错反馈 
			exit;
	...
	public static function getTaocanfee($authorid, $array_taocan, $type, $money,$paytype) {
		...
		switch ($type) {
			case "1" : //  信用卡还款
				...
				$arr_feeset = self :: getcreditcardsetinfo($money,$paytype);
			...
			case "2" : // 转账汇款
				...
				$getarriveid = $arr_feeset['arriveid']; //信用卡还款付款方扣还是收款方扣 
				$slotcardmoney = $arr_feeset['slotcardmoney']; //每笔最大金额
				$datemaxcount = $arr_feeset['datemaxcount']; //每日最大刷卡次数
				if ($money > $slotcardmoney) //支付金额跟最大金额判断 
					{
					$Error = array (
						'rettype' => '200',
				...
				$shijicount = CheckPayQuota::readsuptfmgcount($authorid,$paytype);
				if ($shijicount >= $datemaxcount) //支付金额跟最大金额判断 
					{
					$Error = array (
						'rettype' => '200',
						'retcode' => '200',
						'retmsg' => '刷卡次数受限。每天只能刷卡:' . $datemaxcount . "次"
					);
					$ErrorReponse->reponError($Error);
					exit ();
				}
		  ...
		  case "5" : // 抵用券
				$listid = $bkcardpayfsetid;
				$arr_feeset = self :: getcouponsetinfo($money);
			    $getfeemoney = $arr_feeset['feemoney']; //购买抵用券手续费金额
	...
	private static function getcreditcardsetinfo($money,$paytype) {
		$db = new DB_test();
	...
	private static function getcouponsetinfo($money) {
		...
		if ($db->nf()) {
			$db->next_record();
			$fee = $db->f('fd_couponset_fee'); //固定费率
			$minfee = 0; //固定费率
			$maxfee = $db->f('fd_couponset_maxfee'); //固定费率
		}
        $fee = substr($fee, 0, -1); //浮动手续费
		$feemoney = round(($fee* $money/100), 2);
		//$feemoney = $feemoney < $minfee ? $minfee : $feemoney;
		$feemoney = $feemoney > $maxfee ? $maxfee : $feemoney;

        $file="../../".CONST_LOGDIR."/".date('Y-m-d')."shouxu-log".".txt";
        $filehandle=fopen($file, "a");
        fwrite($filehandle,"\r\n======连接内容：\r\n".$money."----".$fee."---".$feemoney."===".$query."\r\n\r\n\r\n\r\n<!--------------结束------------>\r\n\r\n\r\n");
        fclose($filehandle);
		$defeedirct = 's'; //信用卡还款是收方扣款
		$arr_credcardset = array (
			"feemoney" => $feemoney
		);
		return $arr_credcardset;
 ...
 class CheckPayQuota
{
	...
	public static function readPayQuota($authorid, $paycardid,$paytype,$paymoney) 
	{
		...
		if($paytype=='coupon' || $paytype=='creditcard' || $paytype=='tfmg' || $paytype=='suptfmg' || $paytype=='order' )
		{
			return true;
		}
		$db = new DB_test();
	    ...
	    $arr_monthquota = CheckPayQuota::readmonthnmoney($authorid, $paycardid); //每月已使用额度信息
	    $arr_datequota  = CheckPayQuota::readdatenmoney($authorid, $paycardid,$paydate); //本日已使用额度问题 
	...
	public static function readmonthnmoney($authorid, $paycardid)    //获取每月总额度
	{
		$db = new DB_test();
	...
	public static function readdatenmoney($authorid, $paycardid,$paydate)    //获取每日总额度
	{
		$db = new DB_test();
	...
	public static function readsuptfmgcount($authorid,$paytype)
	{
		...
		$query = "select count(1) as count " .
				 "from tb_agentpaymoneylist  where fd_agpm_authorid = '$authorid' and fd_agpm_paytype  = '$paytype' and " .
				 "fd_agpm_paydate = '$datetime' and fd_agpm_payrq = '00'  ";
		if ($db->execute($query)) {
