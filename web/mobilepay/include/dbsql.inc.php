...
class DB_Sql
{
	...
	var $output = "direct"; ## "direct" (printf message), "re" (return message)
	...
	function connect($Database = "", $Host = "", $User = "", $Password = "") {
		...
		if (0 == $this->Link_ID)
		{
			...
			if (!$this->Link_ID)
			{
				...
				$this->error( "connect ($Host, $User, \$Password) failed" );
				return 0;
			}
	...
	function query( $Query_String )
	{
		...
		if ( $this->Debug ) {
			$this->error( "Debug: query = ".$Query_String."<br>\n" );
	...
	function seek( $pos = 0 ) {
		...
		if ($status)
		{
			...
			$this->error( "seek($pos) failed: result has " . $this->num_rows () . " rows." );
	...
	function nextid($seq_name) {
		...
		if (! $this->locked) {
			if ($this->lock( $this->Seq_Table )) {
				...
				$this->error( "cannot lock " . $this->Seq_Table . " - has it been created?" );
				return 0;
			}
		}
		...
		if (! $this->query( $q )) {
			...
			$this->error( 'query failed in nextid: ' . $q );
			return 0;
		}
	...
	*/
	...
	set_error_handler(array('DB_Sql', 'error_handler'));
